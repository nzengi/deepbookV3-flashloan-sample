...
export const createConfig = (): BotConfig => {
  ...
  const riskLimits: RiskLimits = {
    ...
    maxConcurrentTrades: parseInt(process.env.MAX_CONCURRENT_TRADES || "1"), // Single test trade
  };

  // Mainnet RPC/WS endpoint prioritization
  const suiRpcUrl =
    process.env.SUI_RPC_URL || "https://rpc-mainnet.suiscan.xyz:443";
  const suiWsUrl =
    process.env.SUI_WS_URL || "wss://rpc-mainnet.suiscan.xyz/websocket";
  ...
  const config: BotConfig = {
    ...
    minProfitThreshold: new BigNumber(
    ...
    maxTradeAmount: new BigNumber(
      process.env.MAX_TRADE_AMOUNT || "10000000000000"
    ), // 10,000 SUI
